...
namespace KnowledgeBase.Infrastructure.Repositories
{
    public class TestQuestionAnswerRepository : Repository<TestQuestionAnswer>, ITestQuestionAnswerRepository
    {
        ...
        public async Task<IList<TestQuestionAnswer>> GetByUserId(Guid userId)
        {
            var testQuestionAnswers = await (from testQuestionAnswer in Context.TestQuestionAnswers
                                             join test in Context.Tests on testQuestionAnswer.TestId equals test.Id
                                             where test.UserTests.Any(x => x.UserId.Equals(userId))
                                             select testQuestionAnswer).ToListAsync();
            ...
            using (DbCommand command = Context.Database.GetDbConnection().CreateCommand())
            {
                ...
                using (DbDataReader reader = await command.ExecuteReaderAsync())
                {
                    ...
                    while (await reader.ReadAsync())
                    {
                        ...
                        this.Context.Entry(new TestQuestionAnswer { Id = Guid.NewGuid(), TestId = test.Entity.Id, QuestionId = reader.GetGuid(1), QuestionText = reader.GetString(2), AnswerId = reader.GetGuid(3), AnswerText = reader.GetString(4), CreatedOn = DateTime.UtcNow }).State = EntityState.Added;
